...
const api = {
  ...
  scanRepos: (): Promise<DiscoveredRepo[]> => ipcRenderer.invoke('repos:scan'),
  openScanDirPicker: (): Promise<string | null> => ipcRenderer.invoke('repos:open-scan-dir-picker'),
  resetDb: (): Promise<void> => ipcRenderer.invoke('repos:reset'),
  ...
  showCommit: (
    repoPath: string,
    hash: string
  ): Promise<{ diff: ParsedFile[] } | { error: string }> =>
    ipcRenderer.invoke('commits:show', repoPath, hash),

  // MCP controls
  getMcpStatus: (): Promise<{ running: boolean }> => ipcRenderer.invoke('mcp:get-status'),
  toggleMcp: (): Promise<{ running: boolean }> => ipcRenderer.invoke('mcp:toggle'),

  // Integrations
  getIntegrations: (): Promise<IntegrationStatus[]> => ipcRenderer.invoke('integrations:get'),
  installIntegrations: (): Promise<void> => ipcRenderer.invoke('integrations:install'),
  ...
  onReviewUpdated: (
    callback: (data: { repoPath: string; prId: string; reviewId: string }) => void
  ): (() => void) => {
    const listener = (
      _e: IpcRendererEvent,
      data: { repoPath: string; prId: string; reviewId: string }
    ) => callback(data)
